...
class CamVidDataset(Dataset):
    ...
    def parse_class_dict(self, class_dict_path):
        ...
        class_dict = {}
        for index, row in class_dict_df.iterrows():
            rgb = (row['r'], row['g'], row['b'])
            class_dict[index] = (rgb,row['name'])  # Map RGB to class index
        return class_dict
    ...
    def rgb_to_class_id(self, label_img):
        ...
        for i,(rgb, _) in self.class_dict.items():
            mask = np.all(label == rgb, axis=2)
            class_id[mask] = i
        return Image.fromarray(class_id)
